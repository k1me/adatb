...
def kezeli_create(kezeli: schema_kezeli.KezeliCreate, db: Session):
    db_kezeli = model_kezeli.Kezeli(felhasznalonev=kezeli.felhasznalonev, email=kezeli.email, mettol=kezeli.mettol, meddig=kezeli.meddig)
    db.add(db_kezeli)
    db.commit()
...
def kezeli_by_felhasznalonev(felhasznalonev: str, db: Session):
    db_kezeli = db.query(model_kezeli.Kezeli).filter(model_kezeli.Kezeli.felhasznalonev == felhasznalonev).all()
    if not db_kezeli:
