...
from models import vendeg as model_vendeg, foglalas as model_foglalas
from schemas import vendeg as schema_vendeg


def vendeg_create(vendeg: schema_vendeg.VendegCreate, db: Session):
    db_vendeg = model_vendeg.Vendeg(
        email=vendeg.email,
        nev=vendeg.nev,
        telefonszam=vendeg.telefonszam,
        szuletesi_datum=vendeg.szuletesi_datum,
    )
    db.add(db_vendeg)
    db.commit()
...
def vendeg_delete(email: str, db: Session):
    db_vendeg = (
        db.query(model_vendeg.Vendeg).filter(model_vendeg.Vendeg.email == email).first()
    )
